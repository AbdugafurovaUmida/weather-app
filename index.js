...
async function getWeather() {
  try {
      ...
      const forecastData = response.data.list;
       response.data.city.country ? getCityImage(response.data.city.country) : 'No Unicode'
...
async function getCityImage(cityCode) {
     const cityImage = await axios.post(`https://countriesnow.space/api/v0.1/countries/flag/images`, {
      "iso2" : cityCode
     })
     let countryFlag = document.querySelector('.country-flag__image')
     countryFlag.src = cityImage?.data?.data?.flag
}
